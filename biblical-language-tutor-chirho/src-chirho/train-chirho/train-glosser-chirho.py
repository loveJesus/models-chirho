...
"""
train-glosser-chirho.py
Fine-tunes mT5-small for interlinear glossing of biblical Hebrew and Greek.

Input:  "gloss [greek]: Βίβλος γενέσεως Ἰησοῦ Χριστοῦ [MAT 1:1]"
Output: "[The] book | of [the] genealogy | of Jesus | Christ"
...
OUTPUT_DIR_CHIRHO = BASE_DIR_CHIRHO / "models-chirho" / "glosser-chirho"
...
def compute_metrics_chirho(eval_preds_chirho, tokenizer_chirho):
    """Compute BLEU score and word-level gloss accuracy."""
    ...
    try:
        import sacrebleu
        bleu_result_chirho = sacrebleu.corpus_bleu(
            decoded_preds_chirho,
            [decoded_labels_chirho],
        )
    ...
    # Word-level gloss accuracy: compare pipe-separated glosses
    correct_glosses_chirho = 0
    total_glosses_chirho = 0

    for pred_chirho, label_chirho in zip(decoded_preds_chirho, decoded_labels_chirho):
        pred_words_chirho = [w_chirho.strip() for w_chirho in pred_chirho.split("|")]
        label_words_chirho = [w_chirho.strip() for w_chirho in label_chirho.split("|")]

        for i_chirho, label_word_chirho in enumerate(label_words_chirho):
            total_glosses_chirho += 1
            if i_chirho < len(pred_words_chirho):
                # Normalized comparison (lowercase, strip whitespace)
                if pred_words_chirho[i_chirho].lower().strip() == label_word_chirho.lower().strip():
                    correct_glosses_chirho += 1

    word_accuracy_chirho = correct_glosses_chirho / total_glosses_chirho if total_glosses_chirho > 0 else 0
...
def main_chirho():
    """Main training loop for the interlinear glosser."""
    print("=" * 60)
    print("Biblical Interlinear Glosser Training (mT5-small)")
    print("=" * 60)

    config_chirho = load_config_chirho()
    glosser_config_chirho = config_chirho["glosser_chirho"]
    ...
    model_name_chirho = glosser_config_chirho["model_name_chirho"]
    max_input_length_chirho = glosser_config_chirho["max_input_length_chirho"]
    max_target_length_chirho = glosser_config_chirho["max_target_length_chirho"]
    ...
    train_dataset_chirho = load_jsonl_chirho(DATA_DIR_CHIRHO / "train-glosser-chirho.jsonl")
    val_dataset_chirho = load_jsonl_chirho(DATA_DIR_CHIRHO / "val-glosser-chirho.jsonl")

    print(f"  Train: {len(train_dataset_chirho)} examples")
    print(f"  Validation: {len(val_dataset_chirho)} examples")

    # Subsample eval set for speed (predict_with_generate is slow on MPS)
    max_eval_chirho = glosser_config_chirho.get("max_eval_samples_chirho", 0)
    ...
    use_bf16_chirho = is_cuda_chirho and glosser_config_chirho.get("bf16_chirho", True)
    batch_size_chirho = glosser_config_chirho["batch_size_chirho"]

    if is_mps_chirho:
        batch_size_chirho = min(batch_size_chirho, 4)
    ...
    training_args_chirho = Seq2SeqTrainingArguments(
        ...
        num_train_epochs=glosser_config_chirho["num_epochs_chirho"],
        per_device_train_batch_size=batch_size_chirho,
        per_device_eval_batch_size=batch_size_chirho * 2,
        learning_rate=glosser_config_chirho["learning_rate_chirho"],
        weight_decay=glosser_config_chirho["weight_decay_chirho"],
        warmup_steps=glosser_config_chirho["warmup_steps_chirho"],
        eval_strategy="epoch",
        save_strategy="epoch",
        load_best_model_at_end=True,
        metric_for_best_model="bleu_chirho",
    ...
    trainer_chirho = Seq2SeqTrainer(
        ...
        callbacks=[
            EarlyStoppingCallback(
                early_stopping_patience=glosser_config_chirho["early_stopping_patience_chirho"]
            )
        ],
    )
    ...
    # Train
    print("\nStarting training...")
    print(f"  Epochs: {glosser_config_chirho['num_epochs_chirho']}")
    print(f"  Batch size: {batch_size_chirho}")
    print(f"  Learning rate: {glosser_config_chirho['learning_rate_chirho']}")
    ...
    print(f"  BLEU: {eval_results_chirho.get('eval_bleu_chirho', 'N/A')}")
    print(f"  Word accuracy: {eval_results_chirho.get('eval_word_accuracy_chirho', 'N/A')}")
    ...
    test_inputs_chirho = [
        ...
        'gloss [greek]: Ἐν ἀρχῇ ἦν ὁ λόγος [JHN 1:1]',
    ]
